...
void main() {
  test(
    'Search combines accent-insensitive text, category and platform',
    () async {
      ...
      expect(
        controller.results.map((game) => game.title),
        contains('Hades'),
      );
  ...
  test(
    'Choices finish without repeating and saved games toggle once',
    () async {
      final controller = ExploreController(DemoExploreRepository());
      addTearDown(controller.dispose);
  ...
  testWidgets(
    'All footer positions remain fixed while selection turns purple',
    (tester) async {
  ...
  for (final width in [320.0, 390.0]) {
    for (final scale in [1.0, 2.0]) {
      testWidgets(
        'Explore full page at $width with text scale $scale',
        (tester) async {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        NextPlayApp(
          exploreRepository: DemoExploreRepository(),
          libraryRepository: NoopLibraryRepository(),
          authController: auth,
        ),
      );
      ...
      await tester.tap(
        find.byKey(
          const ValueKey(AppDestination.explore),
        ),
      );
      ...
      expect(
        find.text('1 jogo encontrado'),
        findsOneWidget,
      );

      await tester.ensureVisible(
        find.byType(ExploreGameCard),
      );
      ...
      await tester.tap(
        find.byKey(
          const ValueKey(AppDestination.profile),
        ),
      );
